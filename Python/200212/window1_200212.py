...
# 체크 박스
chk = IntVar()
check_btn_a = Checkbutton(window, text='체크해주세요.', variable=chk, command=checkFunc)

# radio 버튼 : Radiobutton
val = IntVar()
radio_btn_a = Radiobutton(window, text = 'JAVA', variable = val, value = 1, command = radioFunc)
radio_btn_b = Radiobutton(window, text = 'Python', variable = val, value = 2, command = radioFunc)
radio_btn_c = Radiobutton(window, text = 'C', variable = val, value = 3, command = radioFunc)

# 버튼 위젯 : 버튼 표현
btn_close = Button(window, text = '프로그램 종료', fg = 'white', bg = 'gray', command = quit)
